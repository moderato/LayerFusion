...
void benchmark_mkldnn(std::string workload_name,
    ...
    std::vector<float> bias_1_data(inter_channel, 0.0f);
    ...
    if (conv_pd_1.src_desc() != input_mem.get_desc()) {
        ...
        primitives.push_back(dnnl::reorder(input_mem, conv_input_mem));
        pr_args.push_back({{MKLDNN_ARG_FROM, input_mem}, {MKLDNN_ARG_TO, conv_input_mem}});
    }

    if (conv_pd_1.weights_desc() != filter_1_mem.get_desc()) {
        conv_filter_1_mem = dnnl::memory(conv_pd_1.weights_desc(), engine);
        primitives.push_back(dnnl::reorder(filter_1_mem, conv_filter_1_mem));
        pr_args.push_back({{MKLDNN_ARG_FROM, filter_1_mem}, {MKLDNN_ARG_TO, conv_filter_1_mem}});
    }
    ...
    primitives.push_back(dnnl::convolution_forward(conv_pd_1));
    pr_args.push_back(conv_args_1);
    stage_1_pr_index = primitives.size() - 1;

    // Reorder the data if necessary
    if (conv_pd_2.src_desc() != conv_pd_1.dst_desc()) {
        inter_mem = dnnl::memory(conv_pd_1.src_desc(), engine);
        primitives.push_back(dnnl::reorder(inter_mem, conv_inter_mem));
        pr_args.push_back({{MKLDNN_ARG_FROM, inter_mem}, {MKLDNN_ARG_TO, conv_inter_mem}});
        inter_reorder_pr_index = primitives.size() - 1;
    }

    if (conv_pd_2.weights_desc() != filter_2_mem.get_desc()) {
        conv_filter_2_mem = dnnl::memory(conv_pd_2.weights_desc(), engine);
        primitives.push_back(dnnl::reorder(filter_2_mem, conv_filter_2_mem));
        pr_args.push_back({{MKLDNN_ARG_FROM, filter_2_mem}, {MKLDNN_ARG_TO, conv_filter_2_mem}});
    }
    ...
    primitives.push_back(dnnl::convolution_forward(conv_pd_2));
    pr_args.push_back(conv_args_2);
    stage_2_pr_index = primitives.size() - 1;
    ...
    if (conv_pd_2.dst_desc() != output_mem.get_desc()) {
        primitives.push_back(dnnl::reorder(conv_output_mem, output_mem));
        pr_args.push_back({{MKLDNN_ARG_FROM, conv_output_mem}, {MKLDNN_ARG_TO, output_mem}});
    } else
