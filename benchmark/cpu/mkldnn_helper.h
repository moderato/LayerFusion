...
void benchmark_mkldnn(std::string workload_name,
    ...
    if (conv_pd_1.weights_desc() != filter_1_mem.get_desc()) {
        ...
        dnnl::reorder(filter_1_mem, conv_filter_1_mem)
                .execute(engine_stream, filter_1_mem, conv_filter_1_mem);
    }
    ...
    auto conv_prim_1 = dnnl::convolution_forward(conv_pd_1);
    ...
    conv_prim_1.execute(engine_stream, conv_args_1);
    ...
    // if (conv_pd_1.dst_desc() != inter_mem.get_desc()) {
    //     dnnl::reorder(conv_inter_mem, inter_mem)
    //             .execute(engine_stream, conv_inter_mem, inter_mem);
    // } else
    //     inter_mem = conv_inter_mem;

    // // Wait for the computation to finalize.
    // engine_stream.wait();

    // float *a = static_cast<float *>(inter_mem.get_data_handle());
    // for (int i = 0; i < 100; i++) {
    ...
    if (conv_pd_2.src_desc() != conv_pd_1.dst_desc()) {
        auto tmp_mem = dnnl::memory(conv_pd_2.src_desc(), engine);
        dnnl::reorder(tmp_mem, conv_inter_mem)
                .execute(engine_stream, tmp_mem, conv_inter_mem);
    }

    if (conv_pd_2.weights_desc() != filter_2_mem.get_desc()) {
        conv_filter_2_mem = dnnl::memory(conv_pd_2.weights_desc(), engine);
        dnnl::reorder(filter_2_mem, conv_filter_2_mem)
                .execute(engine_stream, filter_2_mem, conv_filter_2_mem);
    }
    ...
    // Create the primitive.
    auto conv_prim_2 = dnnl::convolution_forward(conv_pd_2);
    ...
    // Primitive execution: convolution with ReLU.
    conv_prim_2.execute(engine_stream, conv_args_2);
