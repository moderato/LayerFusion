...
def schedule_conv_conv_fused_nhwc(outs, stages, params, bn_relu1=None, bn_relu2=None):
    ...
    step_num_w = 4
    reduce_split = 16
    intermediate_reuse = 1 # How many 32x32 blocks of 1x1 filter reuse the intermediate data
    num_thread_x = 32
    # --------------------
...
def verify_conv_conv_fused(parameters, dtype="float32", layout="NHWC", print_ir=False, print_src=False, save_data=False, export_code=False):
    ...
    def check_device(device):
        ...
        with tvm.target.create(device):
            s = schedule_conv_conv_fused_nhwc(output_stage, stages, params, 
...
if __name__ == "__main__":
    ...
    for p in parameters:
        verify_conv_conv_fused(p, print_ir=False, print_src=True, save_data=False, export_code=False)